...
#pragma unroll
            for (int i = 0; i < BLOCK_SIZE_M; i += A_TILE_ROW_STRIDE)
            {
                ...
                FETCH_FLOAT4(ldg_a_reg[ldg_index]) = FETCH_FLOAT4(A[OFFSET(A_TILE_ROW_START + i, A_TILE_COL + tile_idx, K)]);
            }
#pragma unroll
            for (int i = 0; i < BLOCK_SIZE_K; i += B_TILE_ROW_STRIDE)
            {
                int ldg_index = i / B_TILE_ROW_STRIDE * 4;
                FETCH_FLOAT4(ldg_b_reg[ldg_index]) = FETCH_FLOAT4(B[OFFSET(tile_idx + B_TILE_ROW_START + i, B_TILE_COL, N)]);
            }
        }
...
#pragma unroll
        ...
        {
// load next tile from shared_memory to register
// load A from shared_memory to register
#pragma unroll
...
#pragma unroll
        for (int thread_x = 0; thread_x < THREAD_SIZE_X; thread_x++)
        {
            FETCH_FLOAT4(C[OFFSET(BLOCK_SIZE_M * by + ty * THREAD_SIZE_Y + thread_y, BLOCK_SIZE_N * bx + tx * THREAD_SIZE_X + thread_x, N)]) = FETCH_FLOAT4(accum[thread_y][thread_x]);
        }
    }
}
...
int main(int argc, char **argv)
{
    ...
    for (int run = 0; run < n_iter; run++)
    {
        ...
        Sgemm<BLOCK_SIZE_M, BLOCK_SIZE_K, BLOCK_SIZE_N, THREAD_SIZE_Y, THREAD_SIZE_X, ENABLE_DOUBLE_BUFFER><<<grid_size, block_size>>>(d_A, d_B, d_C, M, N, K);
    }
